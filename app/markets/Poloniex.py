            ...
            if command == "returnTicker" or command == "return24hVolume":
                ...
                ret = urllib2.urlopen(urllib2.Request(
                    'https://poloniex.com/public?command=' + "returnTradeHistory" + '&currencyPair=' + str(
                        req['currencyPair']) + '&start=' + str(req['start']) + '&stop=' + str(req['stop'])))
                return _read_response(ret)
            elif command == "returnChartData":
                ret = urllib2.urlopen(urllib2.Request(
                    'https://poloniex.com/public?command=' + "returnChartData" + '&currencyPair=' + str(
                        req['currencyPair']) + '&start=' + str(req['start']) + '&stop=' + str(req['stop']) + '&period=' + str(req['period'])))
                return _read_response(ret)
...
#        return self.api_query("returnMarketTradeHistory", {'currencyPair': currency_pair})
    def return_chart_data(self, currency_pair, start, stop, period):
        return self.api_query("returnChartData", {"currencyPair": currency_pair, 'start': start, 'stop':stop, 'period':period})

    def return_market_trade_history(self, currency_pair, start, stop):
        return self.api_query("returnMarketTradeHistory",  {"currencyPair": currency_pair, 'start':start, 'stop':stop})
...
#        return self.api_query('returnTradeHistory', {"currencyPair": currency_pair})
    def return_trade_history(self, currency_pair, start, stop):
        return self.api_query('returnTradeHistory', {"currencyPair": currency_pair, 'start':start, 'stop':stop})
